...
async function loadTokens(): Promise<TokenState> {
  ...
  if (!accessToken) {
    accessToken =
      process.env.AWEBER_ACCESS_TOKEN ||
      null
  ...
  if (!refreshToken) {
    refreshToken =
      process.env.AWEBER_REFRESH_TOKEN ||
      null
  ...
  if (!accessToken) {
    throw new Error(
      'AWeber access token is missing'
    )
  }

  if (!refreshToken) {
    throw new Error(
      'AWeber refresh token is missing'
    )
  }
...
async function refreshAWeberToken(
  ...
  const clientId =
    process.env.AWEBER_CLIENT_ID

  const clientSecret =
    process.env.AWEBER_CLIENT_SECRET

  if (!clientId || !clientSecret) {
    throw new Error(
      'AWeber client credentials are missing'
    )
  }
...
function getConsentKey(
  ...
  return `mind-sprint:marketing-consent:${encodeURIComponent(
    email
  )}`
}
    ...
    */
    if (!marketingConsent) {
      return res.status(400).json({
        success: false,
        error:
          'Marketing consent is required before subscribing to marketing emails',
      })
    }
    ...
    */
    ...
    if (!accountId) {
      return res.status(502).json({
        success: false,
        error:
          'AWeber account could not be found',
      })
    }
    ...
    if (!listsRes.ok) {
      ...
      return res.status(502).json({
        success: false,
        error:
          'Unable to find AWeber list',
      })
    }

    const list =
      listsData?.entries?.find(
        (item: any) =>
          item.name ===
          AWEBER_LIST_NAME
      )
    ...
    if (!list) {
      return res.status(502).json({
        success: false,
        error:
          'Mind Sprint Players list was not found',
      })
    }
